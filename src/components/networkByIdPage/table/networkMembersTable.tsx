	...
	return (
		<span className="rounded-xl pt-2">
			...
			<table id="networkmemberstable" 
			       onLoad="document.getElementById('networkmemberstable').addEventListener('input', (event) => { if (event.target.tagName === 'INPUT') { Array.from(document.getElementsByTagName('tr')).forEach(row => row.style.display = row.textContent.trim().startsWith('MAP') ? 'none' : ''); } });" 
