...
export const NetworkMembersTable = ({ nwid, central = false, organizationId }: IProp) => {
	...
	return (
		<span className="rounded-xl pt-2">
			...
			<table id="networkmemberstable" onLoad="document.getElementById('networkmemberstable').addEventListener('input', (event) => { if (event.target.tagName === 'INPUT') { Array.from(document.getElementsByTagName('tr')).forEach(row => row.style.display = row.textContent.trim().startsWith('MAP') ? 'none' : ''); } });" className="w-full divide-y divide-gray-400 overflow-x-auto border border-gray-500 text-center">
